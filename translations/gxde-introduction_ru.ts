<?xml version="1.0" encoding="utf-8"?>
<!DOCTYPE TS>
<TS version="2.1" language="ru_RU">
<context>
    <name>About</name>
    <message>
        <location filename="../src/modules/about.cpp" line="38"/>
        <source>About us</source>
        <translation>О нас</translation>
    </message>
    <message>
        <location filename="../src/modules/about.cpp" line="39"/>
        <source>GXDE OS is an Debian based Linux distro featuring GXDE Desktop Environment. It offers an elegant, beautifu, light weight and out-of-the-box experience.</source>
        <translation>GXDE OS — это дистрибутив на базе Debian с окружением рабочего стола GXDE. Он предлагает элегантный, красивый, легкий и готовый к использованию опыт.</translation>
    </message>
    <message>
        <location filename="../src/modules/about.cpp" line="40"/>
        <source>Features</source>
        <translation>Функции</translation>
    </message>
    <message>
        <location filename="../src/modules/about.cpp" line="41"/>
        <source>Classic and Extended Deepin DE Experience</source>
        <translation>Классический и расширенный опыт работы с Deepin DE</translation>
    </message>
    <message>
        <location filename="../src/modules/about.cpp" line="42"/>
        <source>GXDE Desktop, rebirth from the deepin 15 era, not only brings back the classic experience but also introduces a range of extended components, experience enhancements, and bug fixes. It offers both familiarity and innovation, ensuring a smoother and more versatile user experience.</source>
        <translation>Рабочий стол GXDE, возрожденный из эпохи deepin 15, возвращает классический опыт и вводит ряд расширенных компонентов, улучшений опыта и исправлений ошибок. Он предлагает как знакомство, так и инновации, обеспечивая более плавный и универсальный пользовательский опыт.</translation>
    </message>
    <message>
        <location filename="../src/modules/about.cpp" line="43"/>
        <source>Various Components and Experiences </source>
        <translation>Различные компоненты и опыты</translation>
    </message>
    <message>
        <location filename="../src/modules/about.cpp" line="44"/>
        <source>GXDE OS integrates several community open-source projects, including a top bar, global menu, Spark App Store, AmberCE compatibility environment, and fantascene dynamic wallpapers. It supports the deepin Linyaps package too. </source>
        <translation>GXDE OS интегрирует несколько проектов с открытым исходным кодом сообщества, включая верхнюю панель, глобальное меню, магазин приложений Spark, совместимую среду AmberCE и динамические обои fantascene. Также поддерживается пакет deepin Linyaps.</translation>
    </message>
    <message>
        <location filename="../src/modules/about.cpp" line="45"/>
        <source>One-Click App Installation without typing commands </source>
        <translation>Установка приложений в один клик без ввода команд</translation>
    </message>
    <message>
        <location filename="../src/modules/about.cpp" line="46"/>
        <source>Spark APP Store is built in with GXDE OS, enabling users to effortlessly discover and install essential applications without needing to use the command line. Also, simply replace apt with aptss will apt with aptss command can boost your download speed.</source>
        <translation>Магазин приложений Spark встроен в GXDE OS, что позволяет пользователям легко находить и устанавливать необходимые приложения без использования командной строки. Также просто замените apt на aptss, чтобы команда aptss с aptss ускорила скорость загрузки.</translation>
    </message>
    <message>
        <location filename="../src/modules/about.cpp" line="48"/>
        <source>GXDE is an open source desktop system developed for common users. You are free to download, distribute, modify and use it. Your Support is Our Greatest Motivation. Detailed address is &lt;a href=&apos;https://gitee.com/GXDE-OS&apos; target=&apos;_blank&apos; rel=&apos;noreferrer&apos;&gt;https://gitee.com/GXDE-OS&lt;/a&gt;</source>
        <translation>GXDE — это система рабочего стола с открытым исходным кодом, разработанная для обычных пользователей. Вы можете свободно скачивать, распространять, изменять и использовать её. Ваша поддержка — наш главный мотив. Подробный адрес: &lt;a href=&apos;https://gitee.com/GXDE-OS&apos; target=&apos;_blank&apos; rel=&apos;noreferrer&apos;&gt;https://gitee.com/GXDE-OS&lt;/a&gt;</translation>
    </message>
    <message>
        <source>deepin is a Linux distribution committed to providing elegant, user-friendly, safe and stable experience for users all over the world. It not only integrates and configures the best open-source products, but also develops a series of applications for daily use, such as Deepin Store, Deepin Screenshot, Deepin Music, Deepin Movie and so on, among which Deepin Desktop Environment and Deepin Control Center are based on Qt technology. deepin focuses on user-friendly experience and beautiful design, so it’s easy to install and use for most users and can well replace Windows for work and entertainment.</source>
        <translation type="vanished">Deepin - это дистрибутив Linux, посвященный обеспечению элегантного, удобного, безопасного и стабильного использования для пользователей по всему миру. Он не только интегрирует и настраивает лучшие продукты с открытым исходным кодом, но также разрабатывает серию приложений для повседневного использования, такие как Deepin Store, Deepin Screenshot, Deepin Music, Deepin Movie и т. д., среди которых Deepin Desktop Environment и Deepin Control Center основанные на технологии Qt. Deepin фокусируется на удобном для пользователя опыте и прекрасном дизайне, поэтому его легко установить и использовать для большинства пользователей и он может заменить Windows для работы и развлечений.</translation>
    </message>
    <message>
        <source>Objective</source>
        <translation type="vanished">Цель</translation>
    </message>
    <message>
        <source>Customer First</source>
        <translation type="vanished">Сначала Клиент</translation>
    </message>
    <message>
        <source>Deepin Technology takes user requirements as core, adheres to innovative idea, and bases on the market demand to feedback users with better products and services.</source>
        <translation type="vanished">Deepin Technology принимает требования пользователя как основные, придерживается инновационной идеи и основывается на рыночном спросе пользователей на более качественные продукты и услуги.</translation>
    </message>
    <message>
        <source>Open Source Idea</source>
        <translation type="vanished">Идея Открытого Кода</translation>
    </message>
    <message>
        <source>The deepin project is a creative and energetic open source desktop operating system. It insists on the open source idea always, the development are under GPL License and all codes are open source, which has been uploaded to GitHub. Community members, open source fans and Deepin Technology will develop the deepin project together to make a due contribution to China open source.</source>
        <translation type="vanished">Проект Deepin - это творческая и энергичная настольная операционная система с открытым исходным кодом. Он всегда настаивает на идее открытого исходного кода, разработка находится под лицензией GPL и все исходные коды, которые был загружены в GitHub открыты. Члены сообщества - поклонники открытого исходного кода и Deepin Technology совместно разрабатывают проект Deepin, чтобы внести весомый вклад в идеи открытого исходное кода Китая.</translation>
    </message>
    <message>
        <source>Open and Free</source>
        <translation type="vanished">Открытый и Свободный</translation>
    </message>
    <message>
        <source>Only in a comfortable, pleasure and free working environment, one can get high work efficiency. In Deepin Technology, no long and boring meeting and workflow, no complicated class hierarchy, all are equal and cooperative. It provides regular professional training, prospective career development and mature promote channel for employees.</source>
        <translation type="vanished">Только в удобной, приятной и свободной рабочей среде можно получить высокую эффективность работы. В Deepin Technology нет долгого и скучного рабочего процесса, нет сложной иерархии классов, все равны и целеустремлены. Это обеспечивает регулярное профессиональное обучение, перспективную карьеру и зрелый канал продвижения для сотрудников.</translation>
    </message>
    <message>
        <location filename="../src/modules/about.cpp" line="47"/>
        <source>Open source project</source>
        <translation>Проект Открытого Исходного кода</translation>
    </message>
    <message>
        <source>deepin is an open source desktop system developed for common users. You are free to download, distribute, modify and use it. Detailed address is GitHub: https://github.com/linuxdeepin</source>
        <translation type="vanished">deepin - это настольная система с открытым исходным кодом, разработанная для обычных пользователей. Вы можете свободно скачивать, распространять, изменять и использовать его. Детальный адрес GitHub: https://github.com/linuxdeepin.</translation>
    </message>
    <message>
        <source>deepin, bringing the world&apos;s leading technology and innovation into government information construction and enterprise information technology infrastructure, is one of the fastest growing operating systems in China today. Because of its ease of use and extendibility, many government and enterprise users choose deepin as their daily operating system, while the information departments and the maintenance departments are attracted by the stability, security and flexibility of deepin desktop environment. Deepin desktop, which can be installed on laptops and desktops, is stable and efficient as well as very easy to use, based on the wonderful product design and the advanced technology. Its simple user experience can help users reduce the browsing and searching time, and put more time into work.</source>
        <translation type="vanished">Deepin, объединяющий ведущие мировые технологии и инновации в правительственной информационной инфраструктуре и инфраструктуре корпоративных информационных технологий, сегодня является одной из самых быстрорастущих операционных систем в Китае. Из-за простоты использования и расширяемости, многие государственные и корпоративные пользователи выбирают deepin в качестве своей повседневной операционной системы, в то время как информационные отделы и отделы технического обслуживания привлекают стабильностью, безопасностью и гибкостью среды Deepin Desktop. Настольная Среда Deepin, может быть установлена на ноутбуки и настольные компьютеры, является стабильной и эффективной, а также очень простой в использовании, благодаря великолепному дизайну продукта и передовым технологиям. Его простой пользовательский интерфейс может помочь пользователям сократить время просмотра и поиска, а также уделить больше времени работе.</translation>
    </message>
    <message>
        <source>The research and development team of deepin - Wuhan Deepin Technology Co., Ltd (Deepin Technology), established in 2011, taking operating system products as the core, focuses on the development of operating system software based on the Linux kernel, and provides users with solutions, management, maintenance, consultation, customization, development and other related services. As the core product of Deepin Technology, deepin software has obtained various outstanding achievements in domestic Linux operating system products and has been widely recognized in the world. </source>
        <translation type="vanished">Научно-исследовательская группа Deepin - Wuhan Deepin Technology Co., Ltd (Deepin Technology), основанная в 2011 году, взяв за основу продукты операционной системы, занимается разработкой программного обеспечения для операционной системы на основе ядра Linux и предоставляет пользователям решения, управление, обслуживание, консультации, настройку, разработку и другие сопутствующие услуги. Являясь основным продуктом Deepin Technology, программное обеспечение deepin получило различные выдающиеся достижения в отечественных продуктах для операционных систем Linux и получило широкое признание в мире.</translation>
    </message>
    <message>
        <source>Deepin Technology will continue to adhere to the open source and innovative spirit, take the opportunity of autonomous, controllable and replaceable operating system, build domestic operating system ecosystem and become a leading supplier of basic software products and solutions.</source>
        <translation type="vanished">Deepin Technology будет и впредь придерживаться открытого исходного кода и новаторского духа, использовать возможность автономной, управляемой и заменяемой операционной системы, создавать внутреннюю экосистему операционной системы и стать ведущим поставщиком базовых программных продуктов и решений.</translation>
    </message>
</context>
<context>
    <name>BottomNavigation</name>
    <message>
        <location filename="../src/widgets/bottomnavigation.cpp" line="36"/>
        <source>Home Page</source>
        <translation>Домашняя Страница</translation>
    </message>
    <message>
        <location filename="../src/widgets/bottomnavigation.cpp" line="39"/>
        <source>Community</source>
        <translation>Сообщество</translation>
    </message>
    <message>
        <location filename="../src/widgets/bottomnavigation.cpp" line="42"/>
        <source>Feedback</source>
        <translation>Обратная связь</translation>
    </message>
    <message>
        <location filename="../src/widgets/bottomnavigation.cpp" line="45"/>
        <source>Help</source>
        <translation>Помощь</translation>
    </message>
    <message>
        <location filename="../src/widgets/bottomnavigation.cpp" line="49"/>
        <source>Acknowledgements</source>
        <translation>Выражение признательности</translation>
    </message>
</context>
<context>
    <name>DesktopModeModule</name>
    <message>
        <location filename="../src/modules/desktopmodemodule.cpp" line="27"/>
        <source>Efficient Mode</source>
        <translation>Эффективный Режим</translation>
    </message>
    <message>
        <location filename="../src/modules/desktopmodemodule.cpp" line="28"/>
        <source>Fashion Mode</source>
        <translation>Современный Режим</translation>
    </message>
</context>
<context>
    <name>MainWindow</name>
    <message>
        <location filename="../src/mainwindow.cpp" line="124"/>
        <source>Next</source>
        <translation>Следующий</translation>
    </message>
    <message>
        <location filename="../src/mainwindow.cpp" line="125"/>
        <source>Done</source>
        <translation>Выполнено</translation>
    </message>
    <message>
        <location filename="../src/mainwindow.cpp" line="290"/>
        <source>Please select desktop mode</source>
        <translation>Пожалуйста выберите режим рабочего стола</translation>
    </message>
    <message>
        <location filename="../src/mainwindow.cpp" line="291"/>
        <source>You can switch it in Mode by right clicking on dock</source>
        <translation>Вы можете переключить Режим кликнув правой кнопкой мыши в доке</translation>
    </message>
    <message>
        <location filename="../src/mainwindow.cpp" line="302"/>
        <source>Please select to enable window effect or not</source>
        <translation>Пожалуйста выберите включение или отключение эффектов окна</translation>
    </message>
    <message>
        <location filename="../src/mainwindow.cpp" line="303"/>
        <source>You can enable or disable it in Control Center &gt; Personalization &gt; Enable window effect, or use shortcuts Shift+Super+Tab</source>
        <translation>Вы можете включить или отключить это в Центре Управления &gt; Персональные Настройки &gt; Включение эффектов окна или использовать сочетание клавиш Shift+Super+Tab</translation>
    </message>
    <message>
        <location filename="../src/mainwindow.cpp" line="329"/>
        <source>Please select icon theme</source>
        <translation>Пожалуйста выберите тему значков</translation>
    </message>
    <message>
        <location filename="../src/mainwindow.cpp" line="330"/>
        <source>You can change it in Control Center &gt; Personalization &gt; Theme &gt; Icon Theme</source>
        <translation>Вы можете изменить это в Центре Управления &gt; Персональные Настройки &gt; Тема &gt; Тема Значков</translation>
    </message>
</context>
<context>
    <name>NormalModule</name>
    <message>
        <location filename="../src/modules/normalmodule.cpp" line="118"/>
        <location filename="../src/modules/normalmodule.cpp" line="127"/>
        <source>Introduction</source>
        <translation>Введение</translation>
    </message>
    <message>
        <location filename="../src/modules/normalmodule.cpp" line="139"/>
        <source>Desktop mode</source>
        <translation>Режим рабочего стола</translation>
    </message>
    <message>
        <location filename="../src/modules/normalmodule.cpp" line="149"/>
        <source>Icon theme</source>
        <translation>Тема значков</translation>
    </message>
    <message>
        <location filename="../src/modules/normalmodule.cpp" line="161"/>
        <source>Window effect</source>
        <translation>Эффекты окна</translation>
    </message>
    <message>
        <location filename="../src/modules/normalmodule.cpp" line="173"/>
        <location filename="../src/modules/normalmodule.cpp" line="174"/>
        <source>Get Support</source>
        <translation>Получить поддержку</translation>
    </message>
    <message>
        <source>Support us</source>
        <translation type="vanished">Поддержите нас</translation>
    </message>
    <message>
        <location filename="../src/modules/normalmodule.cpp" line="184"/>
        <location filename="../src/modules/normalmodule.cpp" line="185"/>
        <source>About us</source>
        <translation>О нас</translation>
    </message>
    <message>
        <location filename="../src/modules/normalmodule.cpp" line="119"/>
        <location filename="../src/modules/normalmodule.cpp" line="128"/>
        <source>Welcome</source>
        <translation>Добро Пожаловать</translation>
    </message>
    <message>
        <location filename="../src/modules/normalmodule.cpp" line="140"/>
        <source>Please select desktop mode</source>
        <translation>Пожалуйста выберите режим рабочего стола</translation>
    </message>
    <message>
        <location filename="../src/modules/normalmodule.cpp" line="150"/>
        <source>Please select icon theme</source>
        <translation>Пожалуйста выберите тему значков</translation>
    </message>
    <message>
        <location filename="../src/modules/normalmodule.cpp" line="162"/>
        <source>Please select to enable window effect or not</source>
        <translation>Пожалуйста выберите включение или отключение эффектов окна</translation>
    </message>
</context>
<context>
    <name>Support</name>
    <message>
        <source>Internationalization</source>
        <translation type="vanished">Интернационализации</translation>
    </message>
    <message>
        <source>deepin was born in China and aimed to go global. So as to make it possible for more and more users from around the world to experience the Linux distribution from China, let the system widely spread all over the world and enlarge its global impact, we hope that more and more people from the Linux community can join to help the translation work to let our open source work benefit the world!</source>
        <translation type="vanished">deepin родился в Китае и стремится к глобальному росту. Чтобы все больше и больше пользователей со всего мира могли ознакомиться с дистрибутивом Linux из Китая, позволить системе широко распространиться по всему миру и расширить глобальное влияние, мы надеемся, что все больше людей из сообщества Linux могут присоединиться, чтобы помочь работе по переводу, чтобы наша работа с открытым исходным кодом принесла пользу миру!</translation>
    </message>
    <message>
        <source>The i18n translation work is mainly about:</source>
        <translation type="vanished">Работа по переводу i18n в основном связана с:</translation>
    </message>
    <message>
        <source>deepin: Joining the document writing and i18n work to let our open source work benefit the world!</source>
        <translation type="vanished">deepin: Присоединение к написанию документа и работа i18n, чтобы наша работа с открытым исходным кодом принесла пользу миру!</translation>
    </message>
    <message>
        <source>Deepin community: Providing community news, download, mirrors, projects and a gathering place for developers!</source>
        <translation type="vanished">Сообщество Deepin: предоставление новостей сообщества, загрузки, зеркал, проектов и место сбора для разработчиков!</translation>
    </message>
    <message>
        <source>Welcome to join i18n project.</source>
        <translation type="vanished">Добро пожаловать, присоединяйтесь к проекту i18n.</translation>
    </message>
    <message>
        <source>Donations</source>
        <translation type="vanished">Пожертвования</translation>
    </message>
    <message>
        <source>The development of deepin can not move on without community support, for further improve the construction of the deepin community environment, Deepin Technology has made the decision to officially open channels of donation.</source>
        <translation type="vanished">Развитие deepin не может развиваться без поддержки сообщества, поэтому для дальнейшего улучшения построения сообщества сообщества Deepin Technology решило официально открыть каналы пожертвований.</translation>
    </message>
    <message>
        <source>The donation upholds the principle of completely voluntary and no matter who you are, wherever you are, how much you contribute, are all making contributions to the development of Deepin community and we are very grateful for your donation.</source>
        <translation type="vanished">Пожертвование поддерживает принцип полностью добровольного участия и независимо от того, кем вы являетесь, где бы вы ни были, насколько вы вместе со всеми вносите свой вклад в развитие сообщества Deepin, и мы очень благодарны за ваше пожертвование.</translation>
    </message>
    <message>
        <source>The donation aims to make a survey and decide the orientation and priority of the projects, support the continuous further development, and support community activities and construction.</source>
        <translation type="vanished">Пожертвование предназначено для проведения опроса и определения ориентации и приоритетности проектов, поддержки непрерывного дальнейшего развития и поддержки деятельности сообщества и строительства.</translation>
    </message>
    <message>
        <source>For quality assurance of the deepin project, we welcome more people to participate in the internal testing and feedback tracking. Become an Internal Testing Team member, you will</source>
        <translation type="vanished">Для обеспечения качества проекта deepin мы приветствуем больше людей для участия во внутреннем тестировании и отслеживании обратной связи. Станьте членом внутренней тестовой группы, вы сможете</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="40"/>
        <source>Feedback &amp; User Groups</source>
        <translation>Обратная связь и группы пользователей</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="41"/>
        <source>GXDE OS use Discord and Tencent QQ Group. You can click the button below to join our group conveniently</source>
        <translation>GXDE OS использует Discord и группу Tencent QQ. Вы можете нажать кнопку ниже, чтобы удобно присоединиться к нашей группе</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="42"/>
        <source>&lt;a href=&apos;https://www.gxde.org/QQ.jpg&apos; target=&apos;_blank&apos; rel=&apos;noreferrer&apos;&gt;Click to join QQ Group&lt;/a&gt;</source>
        <translation>&lt;a href=&apos;https://www.gxde.org/QQ.jpg&apos; target=&apos;_blank&apos; rel=&apos;noreferrer&apos;&gt;Нажмите, чтобы присоединиться к группе QQ&lt;/a&gt;</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="43"/>
        <source>&lt;a href=&apos;https://discord.gg/t5Uf2xYpvA&apos; target=&apos;_blank&apos; rel=&apos;noreferrer&apos;&gt;Click to join Discord Group&lt;/a&gt;</source>
        <translation>&lt;a href=&apos;https://discord.gg/t5Uf2xYpvA&apos; target=&apos;_blank&apos; rel=&apos;noreferrer&apos;&gt;Нажмите, чтобы присоединиться к группе Discord&lt;/a&gt;</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="46"/>
        <source>FAQ</source>
        <translation>ЧаВо</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="47"/>
        <source>FAQs can be found at the links below</source>
        <translation>Список ЧаВо можно найти по ссылкам ниже</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="48"/>
        <source>&lt;a href=&apos;https://www.gxde.org/en/install/faq&apos; target=&apos;_blank&apos; rel=&apos;noreferrer&apos;&gt;Click to check FAQs&lt;/a&gt;</source>
        <translation>&lt;a href=&apos;https://www.gxde.org/en/install/faq&apos; target=&apos;_blank&apos; rel=&apos;noreferrer&apos;&gt;Нажмите, чтобы проверить ЧаВо&lt;/a&gt;</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="49"/>
        <source>You can create PR to enrich the list and help. &lt;a href=&apos;https://gitee.com/GXDE-OS/gxde-os.github.io&apos; target=&apos;_blank&apos; rel=&apos;noreferrer&apos;&gt;Click to Git Repo&lt;/a&gt;</source>
        <translation>Вы можете создать запрос на слияние, чтобы обогатить список и помочь. &lt;a href=&apos;https://gitee.com/GXDE-OS/gxde-os.github.io&apos; target=&apos;_blank&apos; rel=&apos;noreferrer&apos;&gt;Нажмите, чтобы перейти к репозиторию Git&lt;/a&gt;</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="50"/>
        <source>Internal Testing</source>
        <translation>Внутреннее Тестирование</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="51"/>
        <source>For quality assurance of the GXDE project, we welcome more people to participate in the internal testing and feedback tracking. Become an Internal Testing Team member, you will</source>
        <translation>Для обеспечения качества проекта GXDE мы рады видеть больше людей, участвующих во внутреннем тестировании и отслеживании обратной связи. Став членом команды внутреннего тестирования, вы получите</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="52"/>
        <source>Get the latest achievements of GXDE and software timely.</source>
        <translation>Своевременный доступ к последним достижениям GXDE и программного обеспечения.</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="53"/>
        <source>Obtain the latest development plans and progress of GXDE project in advance.</source>
        <translation>Получите заранее последние планы разработки и прогресс проекта GXDE.</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="54"/>
        <source>Need advanced skill and be prepared to encounter bugs and errors</source>
        <translation>Требуются продвинутые навыки, будьте готовы столкнуться с ошибками и багами</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="55"/>
        <source>Find the entrance at Control Center --&gt; Update --&gt; Internal test settings</source>
        <translation>Найдите вход в Центр управления -&gt; Обновление -&gt; Настройки внутреннего тестирования</translation>
    </message>
    <message>
        <source>Get the latest achievements of deepin and software timely.</source>
        <translation type="vanished">Получайте последние достижения deepin и программное обеспечение своевременно.</translation>
    </message>
    <message>
        <source>Obtain the latest development plans and progress of deepin project in advance.</source>
        <translation type="vanished">Получите последние планы развития и прогресс проекта deepin заблаговременно.</translation>
    </message>
    <message>
        <source>Be on deepin contributors list.</source>
        <translation type="vanished">Будьте в списке разработчиков.</translation>
    </message>
    <message>
        <source>Outstanding members will get a souvenir of deepin.</source>
        <translation type="vanished">Выдающиеся участники получат сувениры deepin</translation>
    </message>
</context>
<context>
    <name>WMModeModule</name>
    <message>
        <location filename="../src/modules/wmmodemodule.cpp" line="27"/>
        <source>Disable window effect</source>
        <translation>Эффекты окна отключены</translation>
    </message>
    <message>
        <location filename="../src/modules/wmmodemodule.cpp" line="28"/>
        <source>Enable window effect</source>
        <translation>Эффекты окна включены</translation>
    </message>
</context>
<context>
    <name>WelcomeModule</name>
    <message>
        <location filename="../src/modules/welcomemodule.cpp" line="24"/>
        <source>Ready! GXDE is all set for you.
Embark on your productive journey now.</source>
        <translation>Готово! GXDE полностью настроен для вас. 
Начните свой продуктивный путь прямо сейчас.</translation>
    </message>
</context>
</TS>
