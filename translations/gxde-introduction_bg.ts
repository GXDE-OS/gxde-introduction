<?xml version="1.0" encoding="utf-8"?>
<!DOCTYPE TS>
<TS version="2.1" language="bg_BG">
<context>
    <name>About</name>
    <message>
        <location filename="../src/modules/about.cpp" line="38"/>
        <source>About us</source>
        <translation>За нас</translation>
    </message>
    <message>
        <location filename="../src/modules/about.cpp" line="39"/>
        <source>GXDE OS is an Debian based Linux distro featuring GXDE Desktop Environment. It offers an elegant, beautifu, light weight and out-of-the-box experience.</source>
        <translation>GXDE OS е дистрибуция на Linux базирана на Debian с графичен интерфейс GXDE. Оферва изискан, красив, лекотежен и готов за използване опит.</translation>
    </message>
    <message>
        <location filename="../src/modules/about.cpp" line="40"/>
        <source>Features</source>
        <translation>Функции</translation>
    </message>
    <message>
        <location filename="../src/modules/about.cpp" line="41"/>
        <source>Classic and Extended Deepin DE Experience</source>
        <translation>Класичен и разширен опит от Deepin DE</translation>
    </message>
    <message>
        <location filename="../src/modules/about.cpp" line="42"/>
        <source>GXDE Desktop, rebirth from the deepin 15 era, not only brings back the classic experience but also introduces a range of extended components, experience enhancements, and bug fixes. It offers both familiarity and innovation, ensuring a smoother and more versatile user experience.</source>
        <translation>GXDE Desktop, възраждан от ерата на Deepin 15, не само връща класическия опит, но и представя широк спектър от разширени компоненти, подобрения в опита и поправки на грешки. Оферва както познатост, така и иновации, осигурявайки по-гладък и по-разнообразен потребителски опит.</translation>
    </message>
    <message>
        <location filename="../src/modules/about.cpp" line="43"/>
        <source>Various Components and Experiences </source>
        <translation>Различни компоненти и опити</translation>
    </message>
    <message>
        <location filename="../src/modules/about.cpp" line="44"/>
        <source>GXDE OS integrates several community open-source projects, including a top bar, global menu, Spark App Store, AmberCE compatibility environment, and fantascene dynamic wallpapers. It supports the deepin Linyaps package too. </source>
        <translation>GXDE OS интегрира няколко общински проекта с отворен код, включително горна лента, глобално меню, магазин за приложения Spark, съвместима среда AmberCE и динамични тапетни изображения от Fantascene. Поддържа и пакета deepin Linyaps.</translation>
    </message>
    <message>
        <location filename="../src/modules/about.cpp" line="45"/>
        <source>One-Click App Installation without typing commands </source>
        <translation>Инсталиране на приложение с един клик без да се въвеждат команди</translation>
    </message>
    <message>
        <location filename="../src/modules/about.cpp" line="46"/>
        <source>Spark APP Store is built in with GXDE OS, enabling users to effortlessly discover and install essential applications without needing to use the command line. Also, simply replace apt with aptss will apt with aptss command can boost your download speed.</source>
        <translation>Магазинът за приложения Spark е вграден в GXDE OS, позволявайки на потребителите да откриват и инсталират лесно необходимите приложения без да използват командния ред. Освен това, просто заменете apt с aptss командата може да ускори скоростта на изтегляне.</translation>
    </message>
    <message>
        <location filename="../src/modules/about.cpp" line="48"/>
        <source>GXDE is an open source desktop system developed for common users. You are free to download, distribute, modify and use it. Your Support is Our Greatest Motivation. Detailed address is &lt;a href=&apos;https://gitee.com/GXDE-OS&apos; target=&apos;_blank&apos; rel=&apos;noreferrer&apos;&gt;https://gitee.com/GXDE-OS&lt;/a&gt;</source>
        <translation>GXDE е система с отворен код за работния плот, разработена за обикновени потребители. Вие можете свободно да изтегляте, разпространявате, модифицирате и използвате я. Поддръжката ви е нашето най-голямо мотивиране. Подробен адрес е &lt;a href=&apos;https://gitee.com/GXDE-OS&apos; target=&apos;_blank&apos; rel=&apos;noreferrer&apos;&gt;https://gitee.com/GXDE-OS&lt;/a&gt;</translation>
    </message>
    <message>
        <source>deepin is a Linux distribution committed to providing elegant, user-friendly, safe and stable experience for users all over the world. It not only integrates and configures the best open-source products, but also develops a series of applications for daily use, such as Deepin Store, Deepin Screenshot, Deepin Music, Deepin Movie and so on, among which Deepin Desktop Environment and Deepin Control Center are based on Qt technology. deepin focuses on user-friendly experience and beautiful design, so it’s easy to install and use for most users and can well replace Windows for work and entertainment.</source>
        <translation type="vanished">deepin е дистрибуция на Линукс, която се стреми да предоставя елегантен, лесен за ползване, безопасна и стабилна работа за потребители от цял ​​свят. Той не само интегрира и конфигурира най-добрите продукти с отворен код, но и разработва серия от приложения за ежедневна употреба, като Deepin Store, Deepin Screenshot, Deepin Music, Deepin Movie и др., сред които Deepin Desktop Environment и Deepin Control Center са базирани на технологията Qt. deepin се фокусира върху леснота при ползване и красив дизайн, затова е лесен за инсталиране и използване за повечето потребители и може да замени Windows за работа и забавление.</translation>
    </message>
    <message>
        <source>Objective</source>
        <translation type="vanished">Цел</translation>
    </message>
    <message>
        <source>Customer First</source>
        <translation type="vanished">Първо потребителски</translation>
    </message>
    <message>
        <source>Deepin Technology takes user requirements as core, adheres to innovative idea, and bases on the market demand to feedback users with better products and services.</source>
        <translation type="vanished">Deepin Technology приема изискванията на потребителите като основни, придържа се към новаторската идея и основава на търсенето на пазара на потребителите с обратна връзка с по-добри продукти и услуги.</translation>
    </message>
    <message>
        <source>Open Source Idea</source>
        <translation type="vanished">Open Source идея</translation>
    </message>
    <message>
        <source>The deepin project is a creative and energetic open source desktop operating system. It insists on the open source idea always, the development are under GPL License and all codes are open source, which has been uploaded to GitHub. Community members, open source fans and Deepin Technology will develop the deepin project together to make a due contribution to China open source.</source>
        <translation type="vanished">Проектът deepin е творческа и енергична операционна система с отворен код за настолни компютри. Той поддържа идеята за отворен код, разработката е под GPL лиценз и всички кодове са с отворен код, които са качени в GitHub. Членовете на общността, феновете на отворения код и Deepin Technology ще развиват съвместно проекта deepin, за да допринесат за китайския отворен код.</translation>
    </message>
    <message>
        <source>Open and Free</source>
        <translation type="vanished">Отворен и свободен</translation>
    </message>
    <message>
        <source>Only in a comfortable, pleasure and free working environment, one can get high work efficiency. In Deepin Technology, no long and boring meeting and workflow, no complicated class hierarchy, all are equal and cooperative. It provides regular professional training, prospective career development and mature promote channel for employees.</source>
        <translation type="vanished">Само в комфортна, спокойна и свободна работна среда може да се постигне висока ефективност на работа. В Deepin Technology, без дълга и скучна среща с работния поток, без сложна йерархия на класовете, всички са еднакви и кооперативни. Той осигурява редовно професионално обучение, перспективно кариерно развитие и зрял канал за насърчаване на служителите.</translation>
    </message>
    <message>
        <location filename="../src/modules/about.cpp" line="47"/>
        <source>Open source project</source>
        <translation>Open source проект</translation>
    </message>
    <message>
        <source>deepin is an open source desktop system developed for common users. You are free to download, distribute, modify and use it. Detailed address is GitHub: https://github.com/linuxdeepin</source>
        <translation type="vanished">deepin е десктоп система с отворен код, разработена за обикновени потребители. Можете свободно да го изтегляте, разпространявате, променяте и използвате. Подробният адрес е GitHub: https://github.com/linuxdeepin</translation>
    </message>
    <message>
        <source>deepin, bringing the world&apos;s leading technology and innovation into government information construction and enterprise information technology infrastructure, is one of the fastest growing operating systems in China today. Because of its ease of use and extendibility, many government and enterprise users choose deepin as their daily operating system, while the information departments and the maintenance departments are attracted by the stability, security and flexibility of deepin desktop environment. Deepin desktop, which can be installed on laptops and desktops, is stable and efficient as well as very easy to use, based on the wonderful product design and the advanced technology. Its simple user experience can help users reduce the browsing and searching time, and put more time into work.</source>
        <translation type="vanished">deepin, въвежда на водещи световни технологии и иновации в управлението на информационните структури конструкции и инфраструктурата на информационните технологии на предприятието, и е една от най-бързо развиващите се операционни системи в Китай днес. Поради лесната си употреба и разширяемост, много правителствени и корпоративни потребители го избират като ежедневна операционна система, докато информационните отдели и отделите по поддръжката са привлечени от стабилността, сигурността и гъвкавостта на десктопната работна среда. Deepin десктоп, който може да се инсталира на лаптопи и настолни компютри, е стабилен и ефективен, както и много лесен за използване, базиран на прекрасен дизайн на продукта и съвременните технологии. Неговата простота на използване може да помогне на потребителите да намалят времето за работа в Интернет и търсене, и да вложат повече време в работата.</translation>
    </message>
    <message>
        <source>The research and development team of deepin - Wuhan Deepin Technology Co., Ltd (Deepin Technology), established in 2011, taking operating system products as the core, focuses on the development of operating system software based on the Linux kernel, and provides users with solutions, management, maintenance, consultation, customization, development and other related services. As the core product of Deepin Technology, deepin software has obtained various outstanding achievements in domestic Linux operating system products and has been widely recognized in the world. </source>
        <translation type="vanished">Екип за научни изследвания и развитие на deepin - Wuhan Deepin Technology Co., Ltd (Deepin Technology), създаден през 2011 г., гледа на  продуктите на операционната система като цялост, фокусира се върху разработването на операционен системен софтуер, базиран на ядрото на Linux, и предоставя на потребителите решения, управление, поддръжка, консултации, персонализиране, развитие и други свързани услуги. Като основен продукт на Deepin Technology, софтуерът на deepin е добавил различни изключителни постижения в продуктите на операционната система Linux и е широко признат в света.</translation>
    </message>
    <message>
        <source>Deepin Technology will continue to adhere to the open source and innovative spirit, take the opportunity of autonomous, controllable and replaceable operating system, build domestic operating system ecosystem and become a leading supplier of basic software products and solutions.</source>
        <translation type="vanished">Deepin Technology ще продължи да се придържа към отворения код и новаторския дух, да се възползва от възможността за самостоятелна, контролируема и заменяема операционна система, да изгради вътрешна екосистема в операционната система и да стане водещ доставчик на основни софтуерни продукти и решения.</translation>
    </message>
</context>
<context>
    <name>BottomNavigation</name>
    <message>
        <location filename="../src/widgets/bottomnavigation.cpp" line="36"/>
        <source>Home Page</source>
        <translation>Начална страница</translation>
    </message>
    <message>
        <location filename="../src/widgets/bottomnavigation.cpp" line="39"/>
        <source>Community</source>
        <translation>Общност</translation>
    </message>
    <message>
        <location filename="../src/widgets/bottomnavigation.cpp" line="42"/>
        <source>Feedback</source>
        <translation>Отзив</translation>
    </message>
    <message>
        <location filename="../src/widgets/bottomnavigation.cpp" line="45"/>
        <source>Help</source>
        <translation>Помощ</translation>
    </message>
    <message>
        <location filename="../src/widgets/bottomnavigation.cpp" line="49"/>
        <source>Acknowledgements</source>
        <translation>Благодарности</translation>
    </message>
</context>
<context>
    <name>DesktopModeModule</name>
    <message>
        <location filename="../src/modules/desktopmodemodule.cpp" line="27"/>
        <source>Efficient Mode</source>
        <translation>Ефективен режим</translation>
    </message>
    <message>
        <location filename="../src/modules/desktopmodemodule.cpp" line="28"/>
        <source>Fashion Mode</source>
        <translation>Модерен режим</translation>
    </message>
</context>
<context>
    <name>MainWindow</name>
    <message>
        <location filename="../src/mainwindow.cpp" line="124"/>
        <source>Next</source>
        <translation>Следващ</translation>
    </message>
    <message>
        <location filename="../src/mainwindow.cpp" line="125"/>
        <source>Done</source>
        <translation>Готово</translation>
    </message>
    <message>
        <location filename="../src/mainwindow.cpp" line="290"/>
        <source>Please select desktop mode</source>
        <translation>Моля, изберете десктоп режим</translation>
    </message>
    <message>
        <location filename="../src/mainwindow.cpp" line="291"/>
        <source>You can switch it in Mode by right clicking on dock</source>
        <translation>Можете да го включите в режим, като щракнете с десния бутон върху дока</translation>
    </message>
    <message>
        <location filename="../src/mainwindow.cpp" line="302"/>
        <source>Please select to enable window effect or not</source>
        <translation>Моля, изберете дали да активирате ефекта на прозореца или не</translation>
    </message>
    <message>
        <location filename="../src/mainwindow.cpp" line="303"/>
        <source>You can enable or disable it in Control Center &gt; Personalization &gt; Enable window effect, or use shortcuts Shift+Super+Tab</source>
        <translation>Можете да разрешите или не в Контролен център &gt; Персонализация&gt; Разреши ефект на прозореца, или използвайте бързи клавиши Shift+Super+Tab</translation>
    </message>
    <message>
        <location filename="../src/mainwindow.cpp" line="329"/>
        <source>Please select icon theme</source>
        <translation>Моля изберете тема за икони</translation>
    </message>
    <message>
        <location filename="../src/mainwindow.cpp" line="330"/>
        <source>You can change it in Control Center &gt; Personalization &gt; Theme &gt; Icon Theme</source>
        <translation>Можете да го промените в Контролен център&gt; Персонализация&gt; Тема&gt; Тема на иконите</translation>
    </message>
</context>
<context>
    <name>NormalModule</name>
    <message>
        <location filename="../src/modules/normalmodule.cpp" line="118"/>
        <location filename="../src/modules/normalmodule.cpp" line="127"/>
        <source>Introduction</source>
        <translation>Въведение</translation>
    </message>
    <message>
        <location filename="../src/modules/normalmodule.cpp" line="139"/>
        <source>Desktop mode</source>
        <translation>Десктоп режим</translation>
    </message>
    <message>
        <location filename="../src/modules/normalmodule.cpp" line="149"/>
        <source>Icon theme</source>
        <translation>Тема на иконите</translation>
    </message>
    <message>
        <location filename="../src/modules/normalmodule.cpp" line="161"/>
        <source>Window effect</source>
        <translation>Ефект на прозореца</translation>
    </message>
    <message>
        <location filename="../src/modules/normalmodule.cpp" line="173"/>
        <location filename="../src/modules/normalmodule.cpp" line="174"/>
        <source>Get Support</source>
        <translation>Получете поддръжка</translation>
    </message>
    <message>
        <source>Support us</source>
        <translation type="vanished">Подкрепете ни</translation>
    </message>
    <message>
        <location filename="../src/modules/normalmodule.cpp" line="184"/>
        <location filename="../src/modules/normalmodule.cpp" line="185"/>
        <source>About us</source>
        <translation>За нас</translation>
    </message>
    <message>
        <location filename="../src/modules/normalmodule.cpp" line="119"/>
        <location filename="../src/modules/normalmodule.cpp" line="128"/>
        <source>Welcome</source>
        <translation>Добре дошли</translation>
    </message>
    <message>
        <location filename="../src/modules/normalmodule.cpp" line="140"/>
        <source>Please select desktop mode</source>
        <translation>Моля, изберете десктоп режим</translation>
    </message>
    <message>
        <location filename="../src/modules/normalmodule.cpp" line="150"/>
        <source>Please select icon theme</source>
        <translation>Моля изберете тема за икони</translation>
    </message>
    <message>
        <location filename="../src/modules/normalmodule.cpp" line="162"/>
        <source>Please select to enable window effect or not</source>
        <translation>Моля, изберете дали да активирате ефекта на прозореца или не</translation>
    </message>
</context>
<context>
    <name>Support</name>
    <message>
        <source>Internationalization</source>
        <translation type="vanished">Интернационализация</translation>
    </message>
    <message>
        <source>deepin was born in China and aimed to go global. So as to make it possible for more and more users from around the world to experience the Linux distribution from China, let the system widely spread all over the world and enlarge its global impact, we hope that more and more people from the Linux community can join to help the translation work to let our open source work benefit the world!</source>
        <translation type="vanished">deepin е роден в Китай и има за цел да стане глобален. За да може все повече и повече потребители от цял ​​свят да преживеят разпространението на Linux от Китай, системата да се разпространява широко по целия свят и се увеличава глобалното й въздействие, ние се надяваме, че все повече хора от общността на Линукс ще се присъединят, за да помогнат на преводаческата работа, така нашата работа с отворен код да бъде от полза за света!</translation>
    </message>
    <message>
        <source>The i18n translation work is mainly about:</source>
        <translation type="vanished">Превода i18n става главно за:</translation>
    </message>
    <message>
        <source>deepin: Joining the document writing and i18n work to let our open source work benefit the world!</source>
        <translation type="vanished">deepin: Присъединете се към писането на документи и i18n проекти, за да позволим на работата ни с отворен код да бъде от полза за света!</translation>
    </message>
    <message>
        <source>Deepin community: Providing community news, download, mirrors, projects and a gathering place for developers!</source>
        <translation type="vanished">Deepin общност: Предоставяме новини от общността, изтегляния, огледала, проекти и сборно място за програмисти!</translation>
    </message>
    <message>
        <source>Welcome to join i18n project.</source>
        <translation type="vanished">Добре дошли в проекта i18n.</translation>
    </message>
    <message>
        <source>Donations</source>
        <translation type="vanished">Дарения</translation>
    </message>
    <message>
        <source>The development of deepin can not move on without community support, for further improve the construction of the deepin community environment, Deepin Technology has made the decision to officially open channels of donation.</source>
        <translation type="vanished">Развитието на deepin не може да продължи без подкрепата на общността, за по-нататъшно подобряване на изграждането на deepin общността, Deepin Technology е взела решение за официално откриване на канали за дарение.</translation>
    </message>
    <message>
        <source>The donation upholds the principle of completely voluntary and no matter who you are, wherever you are, how much you contribute, are all making contributions to the development of Deepin community and we are very grateful for your donation.</source>
        <translation type="vanished">Дарението е за принципа за доброволност, без значение кои сте, откъде сте, с колко допринасяте, всички допринасят за развитието на общността на Deepin и сме много благодарни за вашето дарение.</translation>
    </message>
    <message>
        <source>The donation aims to make a survey and decide the orientation and priority of the projects, support the continuous further development, and support community activities and construction.</source>
        <translation type="vanished">Дарението има за цел да направи проучване и да определи ориентацията и приоритета на проектите, непрекъсната подкрепа за  по-нататъшно развитие и подкрепа на дейностите на общността и изграждането й.</translation>
    </message>
    <message>
        <source>For quality assurance of the deepin project, we welcome more people to participate in the internal testing and feedback tracking. Become an Internal Testing Team member, you will</source>
        <translation type="vanished">За осигуряване на качеството на проекта deepin, приветстваме повече хора да участват във вътрешното тестване и проследяване на обратна връзка. Станете член на екипа за вътрешен тест, ако искате</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="40"/>
        <source>Feedback &amp; User Groups</source>
        <translation>Обратна връзка и потребителски групи</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="41"/>
        <source>GXDE OS use Discord and Tencent QQ Group. You can click the button below to join our group conveniently</source>
        <translation>GXDE OS използва Discord и група на Tencent QQ. Можете да кликнете бутона по-долу, за да се присъедините към нашата група удобно</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="42"/>
        <source>&lt;a href=&apos;https://www.gxde.org/QQ.jpg&apos; target=&apos;_blank&apos; rel=&apos;noreferrer&apos;&gt;Click to join QQ Group&lt;/a&gt;</source>
        <translation>&lt;a href=&apos;https://www.gxde.org/QQ.jpg&apos; target=&apos;_blank&apos; rel=&apos;noreferrer&apos;&gt;Кликнете, за да се присъедините към групата на QQ&lt;/a&gt;</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="43"/>
        <source>&lt;a href=&apos;https://discord.gg/t5Uf2xYpvA&apos; target=&apos;_blank&apos; rel=&apos;noreferrer&apos;&gt;Click to join Discord Group&lt;/a&gt;</source>
        <translation>&lt;a href=&apos;https://discord.gg/t5Uf2xYpvA&apos; target=&apos;_blank&apos; rel=&apos;noreferrer&apos;&gt;Кликнете, за да се присъедините към групата на Discord&lt;/a&gt;</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="46"/>
        <source>FAQ</source>
        <translation>ЧЗВ</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="47"/>
        <source>FAQs can be found at the links below</source>
        <translation>ЧЗВ могат да бъдат намерени по долните връзки</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="48"/>
        <source>&lt;a href=&apos;https://www.gxde.org/en/install/faq&apos; target=&apos;_blank&apos; rel=&apos;noreferrer&apos;&gt;Click to check FAQs&lt;/a&gt;</source>
        <translation>&lt;a href=&apos;https://www.gxde.org/en/install/faq&apos; target=&apos;_blank&apos; rel=&apos;noreferrer&apos;&gt;Кликнете, за да проверите ЧЗВ&lt;/a&gt;</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="49"/>
        <source>You can create PR to enrich the list and help. &lt;a href=&apos;https://gitee.com/GXDE-OS/gxde-os.github.io&apos; target=&apos;_blank&apos; rel=&apos;noreferrer&apos;&gt;Click to Git Repo&lt;/a&gt;</source>
        <translation>Можете да създадете PR, за да обогатете списъка и да помогнете. &lt;a href=&apos;https://gitee.com/GXDE-OS/gxde-os.github.io&apos; target=&apos;_blank&apos; rel=&apos;noreferrer&apos;&gt;Кликнете, за да отидете до Git Repo&lt;/a&gt;</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="50"/>
        <source>Internal Testing</source>
        <translation>Вътрешно тестване</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="51"/>
        <source>For quality assurance of the GXDE project, we welcome more people to participate in the internal testing and feedback tracking. Become an Internal Testing Team member, you will</source>
        <translation>За осигуряване на качеството на проекта GXDE, добре дошли са повече хора да участват в внутренните тестове и следенето на обратната връзка. Ставайки член на екипа за внутренни тестове, ще</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="52"/>
        <source>Get the latest achievements of GXDE and software timely.</source>
        <translation>Получавате своевременно последните постижения на GXDE и софтуера.</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="53"/>
        <source>Obtain the latest development plans and progress of GXDE project in advance.</source>
        <translation>Получавате предварително последните планове за разработка и напредък на проекта GXDE.</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="54"/>
        <source>Need advanced skill and be prepared to encounter bugs and errors</source>
        <translation>Трябва да имате напреднали умения и да сте готови да срещнете грешки и проблеми</translation>
    </message>
    <message>
        <location filename="../src/modules/support.cpp" line="55"/>
        <source>Find the entrance at Control Center --&gt; Update --&gt; Internal test settings</source>
        <translation>Намерете входа в Контролен център --&gt; Обновяване --&gt; Настройки за внутренни тестове</translation>
    </message>
    <message>
        <source>Get the latest achievements of deepin and software timely.</source>
        <translation type="vanished">Научете най-новите постижения на deepin и софтуер.</translation>
    </message>
    <message>
        <source>Obtain the latest development plans and progress of deepin project in advance.</source>
        <translation type="vanished">Получавайте предварително най-новите планове за развитие и напредъка на проекта deepin.</translation>
    </message>
    <message>
        <source>Be on deepin contributors list.</source>
        <translation type="vanished">Бъдете в списък с deepin сътрудници.</translation>
    </message>
    <message>
        <source>Outstanding members will get a souvenir of deepin.</source>
        <translation type="vanished">Изключителни членове ще получат сувенир от deepin.</translation>
    </message>
</context>
<context>
    <name>WMModeModule</name>
    <message>
        <location filename="../src/modules/wmmodemodule.cpp" line="27"/>
        <source>Disable window effect</source>
        <translation>Деактивиране на ефектите на прозореца</translation>
    </message>
    <message>
        <location filename="../src/modules/wmmodemodule.cpp" line="28"/>
        <source>Enable window effect</source>
        <translation>Разреши ефект на прозореца</translation>
    </message>
</context>
<context>
    <name>WelcomeModule</name>
    <message>
        <location filename="../src/modules/welcomemodule.cpp" line="24"/>
        <source>Ready! GXDE is all set for you.
Embark on your productive journey now.</source>
        <translation>Готово! GXDE е готов за вас. 
Започнете вашия продуктивен път сега.</translation>
    </message>
</context>
</TS>
